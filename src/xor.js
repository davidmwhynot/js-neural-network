...
module.exports = () => {
	...
	network.setInputs([0, 0]);
	network.calculate();
	let zeroOutputs2 = network.getOutputs();
	console.log(zeroOutputs2);
	console.log('=========');
	...
	for (let j = 0; j < 1; ++j) {
		for (let i = 0; i < 1; ++i) {
			// for (let j = 0; j < 1; ++j) {
			// 	for (let i = 0; i < 1; ++i) {
			network.train(shuffle(trainingData), 1, j, false, false);
			// network.train(shuffle(trainingData), 4, j, false, true);
	...
	for (let j = 0; j < 1; ++j) {
		for (let i = 0; i < 1; ++i) {
			// for (let j = 0; j < 1; ++j) {
			// 	for (let i = 0; i < 1; ++i) {
			network.trainGPU(shuffle(trainingData), 4, j, false, false);
			// network.train(shuffle(trainingData), 4, j, false, true);
			// network.train3(shuffle(trainingData), true);
			// network.train3(trainingData, true);
			// network.train(trainingData);
